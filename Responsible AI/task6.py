# Task 6: Fairness in Loan Approval SystemIdentified Fairness Risks:
# If an AI model trains on historical loan data, it may inadvertently learn to deny loans based on age, marital status, or postal codes (which historically correlate with race or religion due to redlining).  Examples of Biased Rules:Age Bias: if age > 65: reject() - Discriminates against the elderly.  Proxy Bias: if zip_code == "90210": approve() - Uses location as a proxy for wealth or race, violating fair lending laws.  
